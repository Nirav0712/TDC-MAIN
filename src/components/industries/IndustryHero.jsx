...
export const IndustryHero = ({ variant, eyebrow, headline, description, visual: Visual, ctaText = 'Start a Project', ctaLink = '/contact' }) => {
  return (
    <section className={`relative pt-[100px] pb-16 lg:pt-[160px] lg:pb-32 px-1 lg:px-1 max-w-[1400px] mx-auto grid grid-cols-1 xl:grid-cols-2 gap-10 lg:gap-16 items-center overflow-x-hidden ${bgThemes[variant] || 'bg-[#F7FAFC]'}`}>
      <div className='flex flex-col z-10 relative'>
        ...
        <div className='flex'>
          <Link to={ctaLink} className='bg-[#061A2E] text-white px-2 py-4 rounded-xl font-bold text-[16px] hover:bg-[#087EA4] transition-all shadow-lg flex items-center justify-center gap-2 w-full sm:w-auto'>
            {ctaText}
          </Link>
        </div>
      </div>
