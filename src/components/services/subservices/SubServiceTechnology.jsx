...
export const SubServiceTechnology = ({ technologies, theme }) => (
    <section className="py-24 bg-white px-1 border-y border-black/5">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center gap-12">
            ...
            <div className="w-full md:w-2/3 flex flex-wrap gap-3">
                {technologies.map((tech, i) => (
                    <span key={i} className={"px-1 py-2.5 rounded-full border border-black/10 text-sm font-medium bg-white shadow-sm hover:scale-105 transition-transform cursor-default " + theme.accent}>
                        {tech}
                    </span>
                ))}
            </div>
