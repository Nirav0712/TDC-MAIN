    ...
    return (
        <div className="flex w-full h-full min-h-[440px]">
            ...
            <div className="w-[340px] shrink-0 bg-white border-r border-brand-border p-6 flex flex-col gap-2">
                <h4 className="text-xs font-bold tracking-widest uppercase text-brand-text-muted mb-4 px-4 sm:px-6">Company</h4>
                {data.items.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            className={`flex flex-col relative px-4 sm:px-6 py-3.5 rounded-xl transition-all duration-300 group ${isActive ? 'bg-brand-primary-navy shadow-md' : 'hover:bg-brand-bg-light'}`}
                        >
                        ...
                        <div className="flex flex-col max-w-lg justify-center">
                            ...
                            {currentItem.valueIndicators && (
                                <div className="grid grid-cols-2 gap-3 mb-8">
                                    {currentItem.valueIndicators.map((val, idx) => (
                                        <div key={idx} className="flex items-center gap-2 bg-white px-4 sm:px-6 py-2.5 rounded-lg border border-brand-border">
                            ...
                            <div className={currentItem.valueIndicators ? "mt-2" : "mt-6"}>
                                <Link
                                    to={currentItem.href}
                                    onClick={() => setActiveMenu(null)}
                                    className="inline-flex items-center gap-2 text-sm font-bold text-white bg-brand-primary-navy px-3.5 py-3.5 rounded-full hover:bg-brand-deep-blue hover:shadow-lg hover:shadow-brand-primary-navy/20 transition-all duration-300 group"
                                >
                        ...
                        <div className="hidden lg:flex flex-1 items-center justify-end relative max-w-[280px]">
                            ...
                            {currentItem.id === 'our-team' && (
                                <div className="w-64 h-64 rounded-3xl border-4 border-white bg-brand-primary-navy/5 shadow-lg flex flex-col justify-center items-center gap-4 relative overflow-hidden">
                                    ...
                                    <span className="text-sm font-bold text-brand-primary-navy z-10 px-3.5 py-1.5 bg-white rounded-full shadow-sm mt-2">40+ Experts</span>
                                </div>
                            )}
                        </div>
                    </motion.div>
